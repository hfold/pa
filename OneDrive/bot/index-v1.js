...
async function romaGet(address) {
    let args;
    try { args = standardPrincipalCV(address); }
    catch (e) {
        console.log(e)
        return 0;
    }
    const config = {
        method: "post",
        url: `https://stacks-node-api.stacks.co/v2/contracts/call-read/SP2A665S3H6FVMZSY4VJ17ESXX21CGS0A32984B1H/romastaking/is-staking-address`,
        data: {
	...
	let numbers
	try { numbers = Number(cv.value.data['staking-data'].value.data.amount.value) / 1000000; }
    catch (e) {
        console.log(e)
        numbers = 0;
    }
    //let numbers = Number(cv.value.data['staking-data'].value.data.amount.value) / 1000000
    ...
    console.log(`Roma coins wallet : ${(parseInt((romaData.data.fungible_tokens[romaKey].balance)) / 1000000)}`)
    console.log(`Roma coins staked: ${numbers}`)
    return (parseInt((romaData.data.fungible_tokens[romaKey].balance)) / 1000000)+numbers
...
async function nftRoleGiver(message) {
    ...
    if (message.author != bot.user) {
        try {
            ...
            // embed.setDescription("Please Put a Proper Wallet");
...
async function roleGiver(message) {
    ...
    if (message.author != bot.user) {
        try {
            ...
            userId = await (await bot.guilds.fetch(Guild)).members.fetch(papu);
            let principalA = contract
			console.log(`principal: ${principalA}`)
            romaCoin = await romaGet(principalA)
			console.log(`romaCoin: ${romaCoin}`)
        }
        catch (e) {
            // console.log(e)
            embed.setColor("#17dada");
            embed.setDescription("Please Put a Proper Wallet - romaget");
            message.channel.send({ embeds: [embed] });
...
bot.on("messageCreate", async (message) => {
    let embed = new discord.MessageEmbed()
    if (message.channel.id == "969676035236958248" || message.channel.id == "957949441501982750"  ) {
        try {
            roleGiver(message);
